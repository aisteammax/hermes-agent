...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def edit_message(
        ...
        *,
        ...
        try:
            if not finalize:
                # Format intermediate edits too, preserving formatting
                # across the streaming timeline.  Without parse_mode,
                # Telegram replaces all message entities with plain text
                # on every intermediate edit, so the user sees raw
                # **markdown** markers until the finalize call.
                formatted = self.format_message(content)
                try:
                    ...
                    logger.debug(
                        "[%s] MarkdownV2 intermediate edit failed, falling back to plain text: %s",
                        self.name, fmt_err,
                    )
                    await self._bot.edit_message_text(
                        chat_id=int(chat_id),
    ...
    async def send_multiple_images(
        ...
        for image_url, alt_text in images:
            is_local_gif = image_url.startswith("file://") and image_url.lower().endswith(".gif")
            if is_local_gif or (not image_url.startswith("file://") and self._is_animation_url(image_url)):
    ...
    async def send_animation(
        ...
        try:
            ...
            # Handle local file paths (file://) by opening the file
            if animation_url.startswith("file://"):
                local_path = _unquote(animation_url[7:])
                if not _os.path.exists(local_path):
                    return SendResult(success=False, error=self._missing_media_path_error("Animation", local_path))
